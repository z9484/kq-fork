...
{
    auto& currentFighter = fighter[fighter_index];
    currentFighter.fighterHealth += amt;
    if (currentFighter.fighterHealth > currentFighter.fighterMaxHealth)
    {
        currentFighter.fighterHealth = currentFighter.fighterMaxHealth;
    }
    if (currentFighter.fighterHealth < 0)
    {
        currentFighter.fighterHealth = 0;
    }
}
...
{
    auto& currentFighter = fighter[fighter_index];
    currentFighter.fighterMagic += amt;
    if (currentFighter.fighterMagic > currentFighter.fighterMaxMagic)
    {
        currentFighter.fighterMagic = currentFighter.fighterMaxMagic;
    }
    if (currentFighter.fighterMagic < 0)
    {
        currentFighter.fighterMagic = 0;
    }
}
...
static void beffect_all_enemies(size_t caster_fighter_index, size_t spell_number)
{
    ...
    if (caster_fighter_index < MAX_PARTY_SIZE)
    {
        ...
        end_fighter_index = kqCombat.num_enemies;
    }
    else
    {
    ...
    {
        kqCombat.ta[fighter_index] = NODISPLAY;
    }
    ...
    case M_SLOW:
        ...
        {
            auto& currentFighter = fighter[fighter_index];
            if (res_throw(fighter_index, magic[spell_number].elem) == 0 && non_dmg_save(fighter_index, sp_hit) == 0 && currentFighter.fighterSpellEffectStats[S_STONE] == 0)
            {
                if (currentFighter.fighterSpellEffectStats[S_TIME] == 2)
                {
                    currentFighter.fighterSpellEffectStats[S_TIME] = 0;
                }
                else
                {
                    if (currentFighter.fighterSpellEffectStats[S_TIME] == 0)
                    {
                        currentFighter.fighterSpellEffectStats[S_TIME] = 1;
                        kqCombat.ta[fighter_index] = NODISPLAY;
                    }
                    else
                    {
                        kqCombat.ta[fighter_index] = MISS;
                    }
                }
            }
            else
    ...
    case M_MALISON:
        ...
        {
            auto& currentFighter = fighter[fighter_index];
            if (non_dmg_save(fighter_index, sp_hit) == 0 && currentFighter.fighterSpellEffectStats[S_MALISON] == 0 && currentFighter.fighterSpellEffectStats[S_STONE] == 0)
            {
                currentFighter.fighterSpellEffectStats[S_MALISON] = 2;
                kqCombat.ta[fighter_index] = NODISPLAY;
            }
            else
            {
    ...
    case M_SLEEPALL:
        ...
        {
            auto& currentFighter = fighter[fighter_index];
            if (res_throw(fighter_index, magic[spell_number].elem) == 0 && non_dmg_save(fighter_index, sp_hit) == 0 && currentFighter.fighterSpellEffectStats[S_SLEEP] == 0 && currentFighter.fighterSpellEffectStats[S_STONE] == 0)
            {
                currentFighter.fighterSpellEffectStats[S_SLEEP] = kqrandom->random_range_exclusive(4, 6);
                kqCombat.ta[fighter_index] = NODISPLAY;
            }
            else
            {
...
static void beffect_one_enemy(size_t caster_fighter_index, size_t target_fighter_index, size_t spell_number)
{
    ...
    kqCombat.ta[target_fighter_index] = NODISPLAY;
    auto& targetFighter = fighter[target_fighter_index];
    if (targetFighter.fighterSpellEffectStats[S_STONE] > 0)
    {
        kqCombat.ta[target_fighter_index] = MISS;
        return;
    }
    ...
    {
    case M_BLIND:
        if (non_dmg_save(target_fighter_index, sp_hit) == 0 && targetFighter.fighterSpellEffectStats[S_BLIND] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_BLIND] = 1;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
        ...
        break;
    case M_CONFUSE:
        if (non_dmg_save(target_fighter_index, sp_hit) == 0 && targetFighter.fighterSpellEffectStats[S_CHARM] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_CHARM] = kqrandom->random_range_exclusive(3, 6);
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
    ...
    case M_STONE:
        if (non_dmg_save(target_fighter_index, sp_hit) == 0)
        {
            for (stats_index = 0; stats_index < 24; stats_index++)
            {
                ...
                {
                    targetFighter.fighterSpellEffectStats[stats_index] = 0;
                }
            }
            targetFighter.fighterSpellEffectStats[S_STONE] = kqrandom->random_range_exclusive(3, 6);
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
    ...
    case M_DIFFUSE:
        ...
        {
            r = 0;
            if (targetFighter.fighterSpellEffectStats[S_RESIST] > 0)
            {
                targetFighter.fighterSpellEffectStats[S_RESIST] = 0;
                r++;
            }
            if (targetFighter.fighterSpellEffectStats[S_TIME] > 1)
            {
                targetFighter.fighterSpellEffectStats[S_TIME] = 0;
                r++;
            }
            if (targetFighter.fighterSpellEffectStats[S_SHIELD] > 0)
            {
                targetFighter.fighterSpellEffectStats[S_SHIELD] = 0;
                r++;
            }
            if (targetFighter.fighterSpellEffectStats[S_BLESS] > 0)
            {
                targetFighter.fighterSpellEffectStats[S_BLESS] = 0;
                r++;
            }
            if (targetFighter.fighterSpellEffectStats[S_STRENGTH] > 0)
            {
                targetFighter.fighterSpellEffectStats[S_STRENGTH] = 0;
                r++;
            }
            if (r == 0)
            {
        ...
        break;
    case M_HOLD:
        if (non_dmg_save(target_fighter_index, sp_hit) == 0 && targetFighter.fighterSpellEffectStats[S_STOP] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_STOP] = kqrandom->random_range_exclusive(2, 5);
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
        ...
        break;
    case M_SILENCE:
        if (non_dmg_save(target_fighter_index, sp_hit) == 0 && targetFighter.fighterSpellEffectStats[S_MUTE] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_MUTE] = 1;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
        ...
        break;
    case M_SLEEP:
        if (non_dmg_save(target_fighter_index, sp_hit) == 0 && targetFighter.fighterSpellEffectStats[S_SLEEP] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_SLEEP] = kqrandom->random_range_exclusive(4, 6);
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
    ...
    case M_ABSORB:
        ...
        r = kqCombat.ta[target_fighter_index];
        if (non_dmg_save(target_fighter_index, sp_hit) == 1)
        {
            r = r / 2;
        }
        if (targetFighter.fighterMagic < abs(r))
        {
            r = 0 - targetFighter.fighterMagic;
        }
        kqCombat.ta[target_fighter_index] = r;
        kqCombat.ta[caster_fighter_index] = 0 - r;
        break;
    case M_DRAIN:
        spell_damage(caster_fighter_index, spell_number, target_fighter_index, 1);
        r = kqCombat.ta[target_fighter_index];
        ...
        if (targetFighter.unl > 0)
        {
            if (fighter[caster_fighter_index].fighterHealth < abs(r))
            {
                r = 0 - fighter[caster_fighter_index].fighterHealth;
            }
            kqCombat.ta[target_fighter_index] = 0 - r;
            kqCombat.ta[caster_fighter_index] = r;
        }
        else
        {
            if (targetFighter.fighterHealth < abs(r))
            {
                r = 0 - targetFighter.fighterHealth;
            }
            kqCombat.ta[target_fighter_index] = r;
            kqCombat.ta[caster_fighter_index] = 0 - r;
        }
    ...
    case M_DOOM:
        if (non_dmg_save(target_fighter_index, sp_hit) == 0)
        {
            a = targetFighter.fighterHealth * 3 / 4;
            if (a < 1)
            {
                a = 1;
            }
            if (targetFighter.fighterHealth - a < 1)
            {
                a = targetFighter.fighterHealth - 1;
            }
            kqCombat.ta[target_fighter_index] = 0 - a;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
    ...
    case M_DEATH:
        ...
        break;
    case M_NAUSEA:
        if (non_dmg_save(target_fighter_index, sp_hit) == 0 && targetFighter.fighterSpellEffectStats[S_MALISON] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_MALISON] = 1;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
...
{
    auto& currentFighter = fighter[fighter_index];
    int temp_int = currentFighter.fighterStats[A_INT];
    int temp_sag = currentFighter.fighterStats[A_SAG];
    int temp_aur = currentFighter.fighterStats[A_AUR];
    int temp_spi = currentFighter.fighterStats[A_SPI];

    currentFighter.fighterStats[A_INT] = sag_int_value;
    currentFighter.fighterStats[A_SAG] = sag_int_value;
    currentFighter.fighterStats[A_AUR] = 100;
    currentFighter.fighterStats[A_SPI] = 100;
    currentFighter.csmem = target_item;
    currentFighter.ctmem = tgt;
    if (tgt == TGT_CASTER)
    {
        currentFighter.ctmem = fighter_index;
    ...
    currentFighter.fighterStats[A_INT] = temp_int;
    currentFighter.fighterStats[A_SAG] = temp_sag;
    currentFighter.fighterStats[A_AUR] = temp_aur;
    currentFighter.fighterStats[A_SPI] = temp_spi;
}
...
int combat_spell(size_t caster_fighter_index, int is_item)
{
    ...
    if (spell_number == M_ABSORB || spell_number == M_DRAIN)
    {
        ...
        {
            if (kqCombat.ta[tgt] <= 0)
            {
                display_amount(tgt, eFont::FONT_RED, 0);
                adjust_mp(tgt, kqCombat.ta[tgt]);
                display_amount(caster_fighter_index, eFont::FONT_GREEN, 0);
                adjust_mp(caster_fighter_index, kqCombat.ta[caster_fighter_index]);
            }
            else
            {
        ...
        else
        {
            if (kqCombat.ta[tgt] <= 0)
            {
                ...
                adjust_hp(tgt, kqCombat.ta[tgt]);
                display_amount(caster_fighter_index, eFont::FONT_YELLOW, 0);
                adjust_hp(caster_fighter_index, kqCombat.ta[caster_fighter_index]);
            }
            else
            {
    ...
    else
    {
        if (ss == 0)
        {
            ...
            for (fighter_index = start_fighter_index; fighter_index < start_fighter_index + end_fighter_index; fighter_index++)
            {
                if (kqCombat.ta[fighter_index] == MISS)
                {
                    b++;
                }
            }
    ...
    {
        auto& currentFighter = fighter[fighter_index];
        if (currentFighter.fighterSpellEffectStats[S_DEAD] == eDeathType::NOT_DEAD && currentFighter.fighterHealth <= 0)
        {
            kqCombat.fkill(fighter_index);
            kqCombat.ta[fighter_index] = 1;
            b++;
        }
        else
        {
...
static void cure_oneall_allies(size_t caster_fighter_index, int tgt, size_t spell_number)
{
    ...
    {
        auto& currentFighter = fighter[fighter_index];
        if (currentFighter.fighterSpellEffectStats[S_STONE] == 0 && currentFighter.fighterSpellEffectStats[S_DEAD] == eDeathType::NOT_DEAD)
        {
            z++;
        }
    }
...
static void geffect_all_allies(size_t caster_fighter_index, size_t spell_number)
{
    ...
    case M_BLESS:
        ...
        {
            auto& currentFighter = fighter[fighter_index];
            if (currentFighter.fighterSpellEffectStats[S_BLESS] < 3)
            {
                fighter_hp = currentFighter.fighterMaxHealth / 10;
                if (fighter_hp < 10)
                {
                    fighter_hp = 10;
                }
                currentFighter.fighterHealth += fighter_hp;
                currentFighter.fighterMaxHealth += fighter_hp;
                currentFighter.fighterSpellEffectStats[S_BLESS]++;
                kqCombat.ta[fighter_index] = NODISPLAY;
            }
            else
            {
    ...
    case M_SHIELDALL:
        ...
        {
            auto& currentFighter = fighter[fighter_index];
            if (currentFighter.fighterSpellEffectStats[S_SHIELD] < 2)
            {
                currentFighter.fighterSpellEffectStats[S_SHIELD] = 2;
                kqCombat.ta[fighter_index] = NODISPLAY;
            }
            else
            {
    ...
    case M_DIVINEGUARD:
        ...
        {
            auto& currentFighter = fighter[fighter_index];
            if (currentFighter.fighterSpellEffectStats[S_SHIELD] < 2 || currentFighter.fighterSpellEffectStats[S_RESIST] < 2)
            {
                if (currentFighter.fighterSpellEffectStats[S_SHIELD] < 2)
                {
                    currentFighter.fighterSpellEffectStats[S_SHIELD] = 2;
                }
                if (currentFighter.fighterSpellEffectStats[S_RESIST] < 2)
                {
                    currentFighter.fighterSpellEffectStats[S_RESIST] = 2;
                }
            }
            else
            {
    ...
    case M_QUICKEN:
        ...
        {
            auto& currentFighter = fighter[fighter_index];
            if (currentFighter.fighterSpellEffectStats[S_TIME] != 2 && currentFighter.fighterSpellEffectStats[S_STONE] == 0)
            {
                if (currentFighter.fighterSpellEffectStats[S_TIME] == 1)
                {
                    currentFighter.fighterSpellEffectStats[S_TIME] = 0;
                }
                else
                {
                    currentFighter.fighterSpellEffectStats[S_TIME] = 2;
                    kqCombat.ta[fighter_index] = NODISPLAY;
                }
            }
            else
...
static void geffect_one_ally(size_t target_fighter_index, size_t spell_number)
{
    ...
    {
    case M_TRUEAIM:
        if (targetFighter.fighterSpellEffectStats[S_TRUESHOT] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_TRUESHOT] = 1;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
        ...
        break;
    case M_THROUGH:
        if (targetFighter.fighterSpellEffectStats[S_ETHER] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_ETHER] = 3;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
        ...
        break;
    case M_REGENERATE:
        if (targetFighter.fighterSpellEffectStats[S_REGEN] == 0)
        {
        ...
        break;
    case M_HOLYMIGHT:
        if (targetFighter.fighterSpellEffectStats[S_STRENGTH] < 2)
        {
            targetFighter.fighterSpellEffectStats[S_STRENGTH]++;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
    ...
    case M_SHELL:
        if (targetFighter.fighterSpellEffectStats[S_RESIST] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_RESIST] = 1;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
        ...
        break;
    case M_WALL:
        if (targetFighter.fighterSpellEffectStats[S_RESIST] != 2)
        {
            targetFighter.fighterSpellEffectStats[S_RESIST] = 2;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
    ...
    case M_SHIELD:
        if (targetFighter.fighterSpellEffectStats[S_SHIELD] == 0)
        {
            targetFighter.fighterSpellEffectStats[S_SHIELD] = 1;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
        ...
        break;
    case M_HASTEN:
        if (targetFighter.fighterSpellEffectStats[S_TIME] != 2)
        {
            if (targetFighter.fighterSpellEffectStats[S_TIME] == 1)
            {
                targetFighter.fighterSpellEffectStats[S_TIME] = 0;
            ...
            {
                targetFighter.fighterSpellEffectStats[S_TIME] = 2;
            }
        }
        else
        {
 ...
 * \param   spell_number Spell number
 */
static void heal_one_ally(
    const size_t /*caster_fighter_index*/,
    const size_t target_fighter_index,
    const size_t spell_number)
{
    auto& targetFighter = fighter[target_fighter_index];
    switch (spell_number)
    {
    case M_RESTORE:
        if (targetFighter.fighterSpellEffectStats[S_DEAD] == eDeathType::NOT_DEAD)
        {
            targetFighter.fighterSpellEffectStats[S_POISON] = 0;
            targetFighter.fighterSpellEffectStats[S_BLIND] = 0;
        }
        break;
    case M_RECOVERY:
        if (targetFighter.fighterSpellEffectStats[S_DEAD] == eDeathType::NOT_DEAD)
        {
            targetFighter.fighterSpellEffectStats[S_POISON] = 0;
            targetFighter.fighterSpellEffectStats[S_BLIND] = 0;
            targetFighter.fighterSpellEffectStats[S_CHARM] = 0;
            targetFighter.fighterSpellEffectStats[S_STOP] = 0;
            targetFighter.fighterSpellEffectStats[S_STONE] = 0;
            targetFighter.fighterSpellEffectStats[S_MUTE] = 0;
            targetFighter.fighterSpellEffectStats[S_SLEEP] = 0;
        }
        break;
    case M_LIFE:
        if (targetFighter.fighterSpellEffectStats[S_DEAD] == 1)
        {
            for (size_t stat_index = 0; stat_index < 24; stat_index++)
            {
                targetFighter.fighterSpellEffectStats[stat_index] = 0;
            }
            targetFighter.fighterHealth = 1;
            targetFighter.fighterAttackSpriteFrame = 0;
        }
        else
        {
            kqCombat.ta[target_fighter_index] = MISS;
    ...
    case M_FULLLIFE:
        if (targetFighter.fighterSpellEffectStats[S_DEAD] == 1)
        {
            ...
            targetFighter.fighterHealth = targetFighter.fighterMaxHealth;
            targetFighter.fighterAttackSpriteFrame = 0;
        }
        else
        {
...
{
    fighter[fighter_index].fighterSpellEffectStats[ss] = kqCombat.rcount + 1;
}
...
{
    int average_damage = 1, multiple_targets = 0, number_of_enemies = 0;
    ...
    if (target_index == SEL_ALL_ENEMIES)
    {
        if (caster_index < MAX_PARTY_SIZE)
        {
            ...
            last_target = kqCombat.num_enemies;
            for (size_t fighter_index = MAX_PARTY_SIZE; fighter_index < MAX_PARTY_SIZE + kqCombat.num_enemies; fighter_index++)
            {
                auto& currentFighter = fighter[fighter_index];
                if (currentFighter.fighterSpellEffectStats[S_DEAD] == eDeathType::NOT_DEAD)
                {
                    number_of_enemies++;
                }
            }
    ...
    for (size_t fighter_index = first_target; fighter_index < first_target + last_target; fighter_index++)
    {
        auto& currentFighter = fighter[fighter_index];
        if (currentFighter.fighterSpellEffectStats[S_DEAD] == eDeathType::NOT_DEAD && currentFighter.fighterMaxHealth > 0)
        {
            tempd = status_adjust(fighter_index);
            int b = do_shell_check(fighter_index, average_damage);
            ...
            if (currentFighter.fighterSpellEffectStats[S_STONE] > 0 &&
                rune_type != R_BLACK &&
                rune_type != R_WHITE &&
                rune_type != R_EARTH &&
                rune_type != R_WATER)
            ...
            if (kqCombat.ta[fighter_index] != 0)
            {
                currentFighter.fighterSpellEffectStats[S_SLEEP] = 0;
            }
        }
        else
        {
    ...
    unsigned int fighters_killed = 0;
    for (size_t fighter_index = first_target; fighter_index < first_target + last_target; fighter_index++)
    {
        auto& currentFighter = fighter[fighter_index];
        if (currentFighter.fighterSpellEffectStats[S_DEAD] == eDeathType::NOT_DEAD && currentFighter.fighterHealth <= 0)
...
static void special_spells(size_t caster_fighter_index, size_t spell_number)
{
    ...
    case M_WARP:
        if (in_combat == 1)
        {
            ...
            kqDraw.do_transition(TRANS_FADE_IN, 2);
            kqCombat.combatend = ESCAPED_COMBAT;
        }
        else
        {
 ...
 */
static void spell_damage(
    const size_t caster_fighter_index,
    const int spell_number,
    const size_t start_fighter_index,
    const size_t end_fighter_index)
{
    int a = 0, b = 0, ad = 0, rt = 0;
    ...
    if (numberOfEnemies > 1 && magic[spell_number].tgt != TGT_ENEMY_ALL)
    {
        ad /= numberOfEnemies;
    }
    for (size_t fighter_index = start_fighter_index; fighter_index < start_fighter_index + end_fighter_index; fighter_index++)
    {
        auto& currentFighter = fighter[fighter_index];
        if (currentFighter.fighterSpellEffectStats[S_DEAD] == eDeathType::NOT_DEAD && currentFighter.fighterMaxHealth > 0)
        {
            ...
            if (currentFighter.fighterSpellEffectStats[S_STONE] > 0 && rt != R_BLACK && rt != R_WHITE && rt != R_EARTH && rt != R_WATER)
            {
                b = b / 10;
            }
            kqCombat.ta[fighter_index] = 0 - b;
 ...
 */
KFighter status_adjust(const size_t fighter_index)
{
