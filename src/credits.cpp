...
void display_credits(Raster* double_buffer)
{
	...
	{
		clear_bitmap(wk);
		kDraw.print_font(wk, (wk->width - 8 * strlen(*cc)) / 2, 42, *cc, eFontColor::FONTCOLOR_NORMAL);
		...
		kDraw.print_font(wk, (wk->width - 8 * strlen(*cc)) / 2, 10, *cc, eFontColor::FONTCOLOR_NORMAL);
		ticks = 0;
	}
	else
	{
	...
	kDraw.print_font(double_buffer, (KQ_SCREEN_W - 8 * strlen(pressf1)) / 2, KQ_SCREEN_H - 30, pressf1, eFontColor::FONTCOLOR_NORMAL);
#ifdef KQ_CHEATS
	...
	kDraw.print_font(double_buffer, 80, 40, hasCheatEnabled ? _("*CHEAT MODE ON*") : _("*CHEAT MODE OFF*"), eFontColor::FONTCOLOR_GOLD);
#endif
#ifdef DEBUGMODE
	/* TT: Similarly, if we are in debug mode, we should be warned. */
	kDraw.print_font(double_buffer, 80, 48, _("*DEBUG MODE ON*"), eFontColor::FONTCOLOR_GOLD);
#endif
}
